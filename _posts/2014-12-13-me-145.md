...
Example Systems:
* Disk robo- has the shape of a disk and is characterized by just one parameter, its radius r > 0.  The disk robot does not have an orientation.  Accordingly, the disk robots only motion is translation in the plane: that is, translations in the horizontal and vertical directinos.
