...
Using this key idea, the <cite>obstacle in configuration space</cite> is the expanded rectangle and the <cite>free configuration space</cite> of the disk robot is described in any of the two completely equivalent forms:

1. the set of points of the disk center such that the disk does not intersect the obstacle and is inside the workspace,
...
1. the roadmap is <cite>accessible</cite> if, for each point q<sub>start</sub> in Q<sub>free</sub>, there is an easily computable path from q<sub>start</sub> to some node in the roadmap,
2. similarly, the roadmap is <cite>departable</cite> if, for each point q<sub>goal</sub> in Q<sub>free</sub>, there is an easily computable path from some node in the roadmap to q<sub>goal</sub>,
3. the roadmap is <cite>connected</cite> if, as in graph theory, and two locations of the roadmap are connected by a path in the roadmap,
4. the roadmap is <cite>efficient with factor</cite> &delta; &ge; 1 if, for any two locations in the eroadmap, say <cite>u</cite> and <cite>v</cite>, the path length from <cite>u</cite> to <cite>v</cite> along edges of the roadmap is no longer than &delta; times the length of the shortest path from <cite>u</cite> to <cite>v</cite> in the environment.

##### Visibility Roadmaps

1. the nodes V of the v isibility graph are all <cite>convex vertices</cite> of the polygons O<sub>1</sub>, ..., O<sub>n</sub>,
2. the edges E of the visibility graph are all pairs of vertices that are <cite>visibly connected</cite>. That is, given two nodes <cite>u</cite> and <cite>v</cite> in V, we add the edge {u,v} to the edge set E if the straight line segment between <cite>u</cite> and <cite>v</cite> is not in collision with any obstacle
3. the <cite>edge weight</cite> of an edge {u,v} is given by the length of the segment connecting <cite>u</cite> and <cite>v</cite>
...
1. r-radius rule: fix a radius r > 0 and select all locations p within distace r of q.
2. K-closest rule: select the K closest locations p to q.
3. component-wise K-closeest rule: from each connected component of teh current roadmap, select the K closest locationsp to q.
