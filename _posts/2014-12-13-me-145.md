...
For complex non-convex environments, we use an algorithm to decompose the free workspace into the union of convex subsets, such as triangles, convex quadrilaterals or trapezoids.  The following nomenclature is convenient: <br/>
...
**Input:** two segments described by their respective vertices p<sub>1</sub>,p<sub>2</sub>,and p<sub>3</sub>, p<sub>4</sub> <br/>
...
(i) the nodes V of the visibility graph are all <cite>convex vertices</cite> of the polygons O<sub>1</sub>, ..., O<sub>n</sub>, <br/>
...
(i) r-radius rule: fix a radius r > 0 and select all locations p within distance r of q. <br/>
(ii) K-closest rule: select the K closest locations p to q. <br/>
(iii) component-wise K-closest rule: from each connected component of the current roadmap, select the K closest location p to q. <br/>
...
**Output:** the function plots the two-link manipulator defined by L<sub>1</sub>,L<sub>2</sub>,W,&alpha;,&beta;, and the obstacle defined by (x<sub>o</sub>,y<sub>o</sub>)
...
**Output:** the function returns 1 if the two-link manipulator defined by L<sub>1</sub>,L<sub>2</sub>,W,&alpha;,&beta; collides with the obstacle defined by (x<sub>o</sub>,y<sub>o</sub>) and r. The function returns 0 otherwise.
